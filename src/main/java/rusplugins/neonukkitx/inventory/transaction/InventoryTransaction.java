...
import rusplugins.neonukkitx.event.inventory.InventoryClickEvent;
import rusplugins.neonukkitx.event.inventory.InventoryTransactionEvent;
...
public class InventoryTransaction {
    ...
    public void addAction(InventoryAction action) {
        if (this.invalid) {
            if (NeoNukkitX.DEBUG > 1) Server.getInstance().getLogger().debug("Failed to add " + action.getClass().getSimpleName() + " for " + source.getName() + ": previous run was marked as invalid");
            return;
        }
        ...
        if (action instanceof SlotChangeAction) {
            ...
            if (targetItem.getCount() > targetItem.getMaxStackSize() || sourceItem.getCount() > sourceItem.getMaxStackSize()) {
                this.invalid = true;
                if (NeoNukkitX.DEBUG > 1) Server.getInstance().getLogger().debug("Failed to add SlotChangeAction for " + source.getName() + ": illegal item stack size");
                return;
            }

            if (!slotChangeAction.getInventory().allowedToAdd(targetItem)) {
                this.invalid = true;
                if (NeoNukkitX.DEBUG > 1) Server.getInstance().getLogger().debug("Failed to add SlotChangeAction for " + source.getName() + ": " + slotChangeAction.getInventory().getName() + " inventory doesn't allow item " + targetItem.getId());
                return;
            }
            ...
            if (!source.isCreative()) {
                ...
                if (slot == 36 || slot == 37 || slot == 38 || slot == 39) {
                    if (sourceItem.hasEnchantment(Enchantment.ID_BINDING_CURSE)) {
                        this.invalid = true;
                        if (NeoNukkitX.DEBUG > 1) Server.getInstance().getLogger().debug("Failed to add SlotChangeAction for " + source.getName() + ": armor has binding curse");
                        return;
                    }
                }
            }
    ...
    protected boolean matchItems() {
        ...
        if (!valid && NeoNukkitX.DEBUG > 1) source.getServer().getLogger().debug("!matchItems " + needItems + " / " + haveItems);
        return valid;
    }
