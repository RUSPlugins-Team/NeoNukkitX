...
import rusplugins.neonukkitx.blockentity.BlockEntityContainer;
...
public class SlotChangeAction extends InventoryAction {
    ...
    public boolean isValid(Player source) {
        if (inventory == null || source.closed) {
            if (NeoNukkitX.DEBUG > 1) source.getServer().getLogger().debug("!isValid null");
            return false;
        }
        if (!source.isCreative() && !(inventory instanceof PlayerUIComponent) && !(inventory instanceof PlayerUIInventory) && !inventory.getViewers().contains(source)) {
            if (NeoNukkitX.DEBUG > 1) source.getServer().getLogger().debug("!isValid viewers " + inventory);
            return false;
        }
        if (inventory instanceof PlayerEnderChestInventory && source.getViewingEnderChest() == null) {
            if (NeoNukkitX.DEBUG > 1) source.getServer().getLogger().debug("!isValid no ec open " + inventory);
            return false;
        }
        if (inventory.getHolder() instanceof BlockEntityContainer) {
            if (((BlockEntity) inventory.getHolder()).closed) {
                if (NeoNukkitX.DEBUG > 1) source.getServer().getLogger().debug("!isValid closed " + inventory);
                return false;
            } else if (((BlockEntity) inventory.getHolder()).chunk == null || ((BlockEntity) inventory.getHolder()).chunk.getProvider() == null) {
                if (NeoNukkitX.DEBUG > 1) source.getServer().getLogger().debug("!isValid chunk provider " + inventory);
                return false;
                ...
                if (NeoNukkitX.DEBUG > 1) source.getServer().getLogger().debug("!isValid container distance " + inventory);
                return false;
            }
        }
        if (inventory.getHolder() != source && inventory.getHolder() instanceof Entity) {
            if (((Entity) inventory.getHolder()).closed) {
                ...
                if (NeoNukkitX.DEBUG > 1) source.getServer().getLogger().debug("!isValid entity distance " + inventory);
                return false;
            }
        }
        ...
        if (!valid && NeoNukkitX.DEBUG > 1) source.getServer().getLogger().debug("!isValid " + inventory);
        return valid;
    }
