...
import rusplugins.neonukkitx.entity.item.EntityXPOrb;
...
import rusplugins.neonukkitx.event.block.BlockBreakEvent;
import rusplugins.neonukkitx.event.block.BlockPlaceEvent;
import rusplugins.neonukkitx.event.block.BlockUpdateEvent;
import rusplugins.neonukkitx.event.level.*;
...
import rusplugins.neonukkitx.event.weather.LightningStrikeEvent;
...
import rusplugins.neonukkitx.level.format.ChunkSection;
import rusplugins.neonukkitx.level.format.FullChunk;
import rusplugins.neonukkitx.level.format.LevelProvider;
import rusplugins.neonukkitx.level.format.anvil.Anvil;
import rusplugins.neonukkitx.level.format.generic.BaseChunk;
import rusplugins.neonukkitx.level.format.generic.BaseFullChunk;
import rusplugins.neonukkitx.level.format.generic.EmptyChunkSection;
import rusplugins.neonukkitx.level.generator.Generator;
import rusplugins.neonukkitx.level.generator.GeneratorTaskFactory;
import rusplugins.neonukkitx.level.generator.PopChunkManager;
import rusplugins.neonukkitx.level.generator.task.GenerationTask;
import rusplugins.neonukkitx.level.generator.task.PopulationTask;
import rusplugins.neonukkitx.level.particle.DestroyBlockParticle;
...
import rusplugins.neonukkitx.level.persistence.impl.DelegatePersistentDataContainer;
import rusplugins.neonukkitx.math.*;
import rusplugins.neonukkitx.math.BlockFace.Plane;
import rusplugins.neonukkitx.metadata.BlockMetadataStore;
import rusplugins.neonukkitx.metadata.MetadataValue;
import rusplugins.neonukkitx.metadata.Metadatable;
...
import rusplugins.neonukkitx.scheduler.BlockUpdateScheduler;
...
public class Level implements ChunkManager, Metadatable, GeneratorTaskFactory {
    ...
    public Level(Server server, String name, String path, Class<? extends LevelProvider> provider) {
        ...
        this.chunkTickRadius = Math.min(this.server.getViewDistance(), Math.max(1, this.server.getConfig("chunk-ticking.tick-radius", 2)));
        this.chunksPerTicks = this.server.getConfig("chunk-ticking.per-tick", 20);
     ...
     * @param type ID of the sound from {@link rusplugins.neonukkitx.network.protocol.LevelSoundEventPacket}
    ...
    public Item useBreakOn(Vector3 vector, BlockFace face, Item item, Player player, boolean createParticles) {
        ...
        if (item.isTool() && item.getDamage() >= item.getMaxDurability()) {
            this.addSound(target, rusplugins.neonukkitx.level.Sound.RANDOM_BREAK);
