package rusplugins.neonukkitx.level.format.leveldb.structure;

import rusplugins.neonukkitx.NeoNukkitX;
import rusplugins.neonukkitx.block.Block;
import rusplugins.neonukkitx.level.format.leveldb.BlockStateMapping;
import rusplugins.neonukkitx.level.util.BitArray;
import rusplugins.neonukkitx.level.util.BitArrayVersion;
import rusplugins.neonukkitx.utils.BinaryStream;
                ...
                try {
                    ...
                    if (blockState == null) {
                        ...
                        if (!blockState.isCustom()) {
                            if (NeoNukkitX.DEBUG > 1) log.info("[{}] Updated unmapped block state: {} => {}", chunkBuilder.debugString(), state, blockState.getVanillaState());
                            chunkBuilder.dirty();
                        }

                        if (NeoNukkitX.DEBUG > 1 && blockState.getRuntimeId() == BlockStateMapping.get().getDefaultRuntimeId()) {
                            log.info("[{}] Chunk contains unknown block {}  => {}", chunkBuilder.debugString(), state, updatedState);
                        }
                    }

                    if (NeoNukkitX.DEBUG > 1 && this.palette.contains(blockState)) {
