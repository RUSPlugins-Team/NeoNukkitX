...
import rusplugins.neonukkitx.block.BlockStone;
import rusplugins.neonukkitx.level.ChunkManager;
import rusplugins.neonukkitx.level.biome.Biome;
import rusplugins.neonukkitx.level.biome.BiomeSelector;
...
import rusplugins.neonukkitx.level.generator.noise.vanilla.f.NoiseGeneratorOctavesF;
...
import rusplugins.neonukkitx.level.generator.populator.impl.PopulatorOceanFloor;
import rusplugins.neonukkitx.level.generator.populator.impl.PopulatorOceanRuin;
import rusplugins.neonukkitx.level.generator.populator.impl.PopulatorRavine;
import rusplugins.neonukkitx.level.generator.populator.impl.PopulatorUnderwaterCave;
import rusplugins.neonukkitx.level.generator.populator.impl.PopulatorCoral;
import rusplugins.neonukkitx.level.generator.populator.impl.PopulatorKelp;
...
public class Normal extends Generator {
    ...
    public void init(ChunkManager level, NukkitRandom random) {
        ...
        this.populators = ImmutableList.of(
                ...
                new PopulatorBeach(),
                new PopulatorCrater(),
                new PopulatorVolcano(),
                new PopulatorLake(),
                new PopulatorRiver(),
                new PopulatorWaterfall(),
                new PopulatorRavine(),
                new PopulatorUnderwaterCave(),
                new PopulatorCoral(),
                new PopulatorKelp(),
                new PopulatorOceanFloor(),
    ...
    public void generateChunk(final int chunkX, final int chunkZ) {
            ...
            for (int zSeg = 0; zSeg < 5; ++zSeg) {
                ...
                for (int xSmooth = -2; xSmooth <= 2; ++xSmooth) {
                    for (int zSmooth = -2; zSmooth <= 2; ++zSmooth) {
                        ...
                        // FIX: Symmetric weights — no penalty for higher biomes
                        // This prevents "cut off" mountains at chunk borders
                        float scaledWeight = biomeWeights[xSmooth + 2 + (zSmooth + 2) * 5] / (baseHeight + 2.0F);

                        // Removed asymmetric scaling that caused chunk border artifacts
        ...
        // FIX: Smooth chunk edges by checking for height discontinuities
        // This fills small gaps and removes protrusions at chunk borders
        smoothChunkBorders(chunk, chunkX, chunkZ);
    ...
    /**
     * Smooths chunk borders by removing stone protrusions and filling small gaps.
     * This is a post-processing step that operates only within the current chunk.
     */
    private void smoothChunkBorders(BaseFullChunk chunk, int chunkX, int chunkZ) {
        // Check all 4 borders: north (z=0), south (z=15), west (x=0), east (x=15)
        int[][] borders = {
            {0, 0, 15, 0},   // North edge: x=0..15, z=0
            {0, 15, 15, 15}, // South edge: x=0..15, z=15
            {0, 0, 0, 15},   // West edge: x=0, z=0..15
            {15, 0, 15, 15}  // East edge: x=15, z=0..15
        };

        for (int[] border : borders) {
            int x1 = border[0], z1 = border[1];
            int x2 = border[2], z2 = border[3];

            for (int x = x1; x <= x2; x++) {
                for (int z = z1; z <= z2; z++) {
                    // Find the height at this border position
                    int borderHeight = -1;
                    ...
                    if (borderHeight < 0) continue;

                    // Check inward neighbor (1 block inside chunk)
                    int nx = x, nz = z;
                    if (x == 0) nx = 1;
                    else if (x == 15) nx = 14;
                    if (z == 0) nz = 1;
                    ...
                    for (int y = 254; y > 32; y--) {
                        if (chunk.getBlockId(nx, y, nz) == STONE) {
                            neighborHeight = y;
                            break;
                        }
                    }
                    ...
                    // If border is more than 8 blocks higher than neighbor, 
                    // it's likely a "cut off" mountain — smooth it down
                    if (borderHeight > neighborHeight + 8) {
                        int targetHeight = neighborHeight + 4;
                        for (int y = borderHeight; y > targetHeight; y--) {
                            chunk.setBlockId(x, y, z, AIR);
                        }
                    }

                    // If border is more than 8 blocks lower, fill the gap
                    if (borderHeight < neighborHeight - 8) {
                        int targetHeight = neighborHeight - 4;
                        for (int y = borderHeight + 1; y <= targetHeight; y++) {
                            chunk.setBlockId(x, y, z, STONE);
                        }
                    }
                }
            }
