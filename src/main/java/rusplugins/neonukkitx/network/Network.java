package rusplugins.neonukkitx.network;
...
import rusplugins.neonukkitx.utils.VarInt;
    ...
    public void processInterfaces() {
        for (SourceInterface interfaz : this.interfaces) {
            try {
                ...
                log.fatal(this.server.getLanguage().translateString("neonukkitx.server.networkError", new String[]{interfaz.getClass().getName(), Utils.getExceptionMessage(e)}), e);
            }
        }
    }
    ...
    public void processBatch(byte[] payload, Collection<DataPacket> packets, CompressionProvider compression) throws Exception {
        ...
        while (!stream.feof()) {
            ...
            try {
                ...
                if (NeoNukkitX.DEBUG > 1 && packetId != ProtocolInfo.LOGIN_PACKET && pk.offset < pk.getRawBuffer().length) {
