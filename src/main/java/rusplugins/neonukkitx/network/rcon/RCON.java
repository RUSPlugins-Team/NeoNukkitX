package rusplugins.neonukkitx.network.rcon;

import rusplugins.neonukkitx.Server;
import rusplugins.neonukkitx.command.RemoteConsoleCommandSender;
import rusplugins.neonukkitx.event.server.RemoteServerCommandEvent;
...
public class RCON {
    ...
    public RCON(Server server, String password, String address, int port) {
        if (password.isEmpty()) {
            throw new IllegalArgumentException(server.getLanguage().translateString("neonukkitx.server.rcon.emptyPasswordError"));
        }
        ...
        try {
            ...
            throw new IllegalArgumentException(server.getLanguage().translateString("neonukkitx.server.rcon.startupError"), e);
        }

        server.getLogger().info(server.getLanguage().translateString("neonukkitx.server.rcon.running", new String[]{address, String.valueOf(port)}));
