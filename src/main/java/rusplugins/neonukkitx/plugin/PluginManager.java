...
import rusplugins.neonukkitx.command.PluginCommand;
import rusplugins.neonukkitx.command.SimpleCommandMap;
import rusplugins.neonukkitx.event.*;
...
import rusplugins.neonukkitx.utils.PluginException;
...
import java.util.*;
import rusplugins.neonukkitx.plugin.internal.InternalModule;
...
public class PluginManager {
    ...
    /**
     * Регистрирует внутренний модуль ядра.
     * Внутренние модули не требуют файловой системы и PluginLoader.
     */
    public void registerInternalModule(rusplugins.neonukkitx.plugin.internal.InternalModule module) {
        if (module == null) {
            return;
        }
        module.initInternal(this.server);
        String name = module.getDescription().getName();
        if (this.plugins.containsKey(name)) {
            this.server.getLogger().warning("Internal module '" + name + "' is already registered!");
            return;
        }
        this.plugins.put(name, module);
        this.server.getLogger().info("Internal module " + module.getDescription().getFullName() + " registered");
    }
    ...
    public void enablePlugin(Plugin plugin) {
        if (!plugin.isEnabled()) {
            try {
                ...
                if (plugin instanceof InternalModule) {
